...
public class BrandHandlerImpl implements BrandHandler {
    ...
    @Async
    @Override
    public Future<Long> onboardItemForBrand(Long orderId) throws Exception {
        ...
        if (serviceOnboardSummary != null){
            logger.error("Order data have been already processed, order ID: " + orderId);
        ...
        try {
            ...
            Long summaryId = serviceOnboardSummaryService.save(orderData);
            logger.debug("Order data onboarded successfully, summaryId: " + summaryId);
            return new AsyncResult<Long>(summaryId);
        } catch (Exception e) {
	...
	public Future<BrandServiceResponse> raiseRequestForBrand(BrandServiceRequest brandServiceRequest) throws Exception {
		...
		return new AsyncResult<BrandServiceResponse>(brandServiceResponse);
	}
